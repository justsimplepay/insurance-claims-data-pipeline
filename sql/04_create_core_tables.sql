...
CREATE TABLE IF NOT EXISTS core.customers (
    ...
    CONSTRAINT ck_core_customer_id CHECK (customer_id ~ '^C[0-9]{5}$'),
    CONSTRAINT ck_core_customer_gender CHECK (gender IS NULL OR gender IN ('F', 'M', 'X')),
    CONSTRAINT ck_core_customer_province CHECK (province IN ('SK', 'AB', 'MB', 'ON', 'BC', 'NS', 'PE', 'NL', 'YT', 'NT')),
    CONSTRAINT ck_core_customer_fsa CHECK (fsa IS NULL OR (char_length(fsa) = 3 AND fsa ~ '^[ABCEGHJ-NPRSTVXY][0-9][ABCEGHJ-NPRSTV-Z]'))
);
...
ALTER TABLE core.customers DROP CONSTRAINT IF EXISTS ck_core_customer_fsa;
ALTER TABLE core.customers ADD CONSTRAINT ck_core_customer_fsa CHECK (fsa IS NULL OR (char_length(fsa) = 3 AND fsa ~ '^[ABCEGHJ-NPRSTVXY][0-9][ABCEGHJ-NPRSTV-Z]'));
