...
-- Missing JSON means the physical file is absent. A malformed JSON file is
-- present and is classified separately as JSON_MALFORMED.
...
WHERE NOT EXISTS (
    ...
    WHERE sf.load_id=c.load_id
      AND sf.source_format='json'
      AND staging.normalize_id(regexp_replace(sf.source_name, '\\.json
-- ---------------------------------------------------------------------------
-- 8. Issue counts on staged source rows.
-- ---------------------------------------------------------------------------
